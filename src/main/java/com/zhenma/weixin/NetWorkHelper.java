...
public class NetWorkHelper {
    ...
    public void PostMessage(String sender, String content, String path) {
        ...
        try {
            ...
            // OutputStream os = conn.getOutputStream(); // 获取输出�?
    ...
    public String getURLResponse(String urlString) {
        ...
        try {
            ...
            conn = (HttpURLConnection) url.openConnection(); //使用URL打开�?��链接
            conn.setDoInput(true); //允许输入流，即允许下�?

            //在android中必须将此项设置为false
            conn.setDoOutput(false); //允许输出流，即允许上�?
    ...
    public String getHttpsResponse(String hsUrl,String requestMethod) {
        ...
        try {
            ...
            con.setDoInput(true); //允许输入流，即允许下�?

            //在android中必须将此项设置为false
            con.setDoOutput(false); //允许输出流，即允许上�?
            con.setUseCaches(false); //不使用缓�?
            if(null!=requestMethod && !requestMethod.equals("")) {
                con.setRequestMethod(requestMethod); //使用指定的方�?
            }
            else{
                con.setRequestMethod("GET"); //使用get请求
            }
            is = con.getInputStream();   //获取输入流，此时才真正建立链�?
            InputStreamReader isr = new InputStreamReader(is);
            BufferedReader bufferReader = new BufferedReader(isr);
            String inputLine = "";
            while ((inputLine = bufferReader.readLine()) != null) {
    ...
    public String PostHttpsResponse(String hsUrl,String json) {
        ...
        try {
            ...
            //在android中必须将此项设置为false
            con.setDoOutput(true); //允许输出流，即允许上�?
            con.setUseCaches(false); //不使用缓�?
